...
class ActionsReport(Report):
    ...
    def __capture_injection_details(cls, repository: Repository, entry: dict):
        ...
        designation = "UNKNOWN"
        workflow_url = entry["workflow_url"]
        ...
        if entry["environments"]:
            ...
            for candidate, details in pwn_req["candidates"].items():
                for step in details["steps"]:
                    ...
                    if (
                        details["confidence"]
                        and details["confidence"] in ["MEDIUM", "HIGH"]
                        and designation in ["UNKNOWN", "LOW"]
                    ):
            ...
            len(pwn_req["triggers"]) == 1
            and pwn_req["triggers"][0] == "pull_request_target:labeled"
        ):
            sub_entries = []
            for candidate, details in pwn_req["candidates"].items():
                for step in details["steps"]:
                ...
                if details["gated"]:
                    for step in details["steps"]:
                        if not cls.__check_mutable(repository, entry, step) and (
                            not (
                                "issue_comment" in pwn_req["triggers"]
                                and len(pwn_req["triggers"]) == 1
                            )
                        ):
                            return []
                        else:
