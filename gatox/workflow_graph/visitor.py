...
class GraphVisitor:
    ...
    def is_target_leaf_node(node):
        ...
        return node.get_attrs().get("injectable") or node.get_attrs().get("is_checkout")
    ...
    def can_traverse_gate(node_attributes, path_context):
        ...
        if soft_gate:
            ...
            if (
                path_context.get("has_mutable_checkout", False)
                and path_context.get("trigger") == "pull_request_target:labeled"
            ):
                return True
            else:
                return False
    ...
    def forward_traversal_generator(cls, graph, workflow_nodes):
        ...
        for workflow_node in workflow_nodes:
            initial_context = {
                ...
                "context_is_mutable": False,
            }
            yield from cls.traverse_node(
                cls, graph, workflow_node, [workflow_node], initial_context, visited
            )
    ...
    def traverse_node(cls, graph, node, path_so_far, context, visited):
        ...
        for succ in graph.successors(node):
            if succ not in path_so_far:
                ...
                yield from cls.traverse_node(
                    graph, succ, new_path, context.copy(), visited
                )
