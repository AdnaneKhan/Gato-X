...
def test_add_results_and_append_path_extra():
    ...
    VisitorUtils._add_results(path, results, IssueType.PWN_REQUEST, Confidence.HIGH, Complexity.TOCTOU)
    VisitorUtils._add_results(path, results, IssueType.ACTIONS_INJECTION)
...
async def test_initialize_action_node_extra(monkeypatch):
    ...
    monkeypatch.setattr(VisitorUtils, "initialize_action_node", staticmethod(fake_initialize_action_node))
...
async def test_add_repo_results_extra(monkeypatch):
    monkeypatch.setattr("gatox.workflow_graph.visitors.visitor_utils.CacheManager", lambda: DummyCacheManager())
    monkeypatch.setattr("gatox.workflow_graph.visitors.visitor_utils.ConfigurationManager", lambda: types.SimpleNamespace(NOTIFICATIONS={"SLACK_WEBHOOKS": False}))
    data = {"repo": [DummyResult("repo"), DummyResult("repo2")]}  # repo2 will be skipped as duplicate hash
