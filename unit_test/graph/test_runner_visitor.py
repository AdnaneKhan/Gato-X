...
def mock_cache_manager():
    ...
    with patch("gatox.workflow_graph.visitors.runner_visitor.CacheManager") as MockCache:
...
class TestRunnerVisitor:
    ...
    async def test_find_runner_workflows_simple_node(self, mock_graph, mock_cache_manager):
        ...
        mock_cached_repo.add_self_hosted_workflows.assert_called_once_with(["test-workflow"])

    async def test_find_runner_workflows_workflow_call_node(self, mock_graph, mock_cache_manager):
        ...
        expected = {"owner/repo": {"reusable-workflow", "caller-workflow-1", "caller-workflow-2"}}
        assert result == expected
        mock_cached_repo.add_self_hosted_workflows.assert_called_once_with(["reusable-workflow"])

    async def test_find_runner_workflows_no_cached_repo(self, mock_graph, mock_cache_manager):
    ...
    async def test_find_runner_workflows_multiple_repos(self, mock_graph, mock_cache_manager):
        ...
        mock_graph.get_nodes_for_tags.return_value = [mock_node1, mock_node2, mock_node3]
    ...
    async def test_find_runner_workflows_exception_handling(self, mock_graph, mock_cache_manager):
        ...
        with patch("gatox.workflow_graph.visitors.runner_visitor.logger") as mock_logger:
            result = await RunnerVisitor.find_runner_workflows(mock_graph)
            ...
            mock_logger.warning.assert_any_call("Error processing node: problematic-node")
            mock_logger.warning.assert_any_call(mock_node1.repo_name.side_effect)

    async def test_find_runner_workflows_empty_caller_workflows(self, mock_graph, mock_cache_manager):
